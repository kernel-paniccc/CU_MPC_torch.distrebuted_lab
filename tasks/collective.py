...
def scatter(rank, world_size):
    ...
    if rank == 0:
        tensor_list = [torch.tensor([i], dtype=torch.long) for i in range(world_size)]
        print(f"Rank {rank}: scattering: {tensor_list}")
    else:
        tensor_list = None

    recv_tensor = torch.zeros(1, dtype=torch.long)
    dist.scatter(recv_tensor, scatter_list=tensor_list, src=0, group=group)
